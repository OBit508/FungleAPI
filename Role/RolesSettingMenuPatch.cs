    ...
    internal class RolesSettingMenuPatch
    {
        ...
        public static void LoadChanceTab(ModPlugin plugin, RolesSettingsMenu menu)
        {
            ...
            foreach (ModdedTeam team in customTeams)
            {
                ...
                CategoryHeaderEditRole header = CreateHeader(menu, Utils.Light(team.TeamColor, 0.7f), team.TeamColor, Utils.Dark(team.TeamColor, 0.7f), Utils.Light(team.TeamColor, 0.9f), team.TeamName);
        ...
        public static Vector3 OrganizeRoles(List<RoleBehaviour> roles, Vector3 currentVec, RolesSettingsMenu menu)
        {
            ...
            for (int i = 0; i < roles.Count; i++)
            {
                ...
                option.labelSprite.color = role.TeamColor;
                ...
                cog.SetNewAction(delegate
                {
                    ...
                    if (cRole != null)
                    {
                        for (int i = 0; i < cRole.RoleB.Configs.Count(); i++)
                        {
                            OptionBehaviour op = null;
                            if (cRole.RoleB.Configs[i] is NumConfig f)
                            {
                                op = CreateOption(numberPrefab, cRole, f, AdvancedTab.transform);
                            }
                            else if (cRole.RoleB.Configs[i] is BoolConfig b)
                            {
                                op = CreateOption(togglePrefab, cRole, b, AdvancedTab.transform);
                            }
                            else if (cRole.RoleB.Configs[i] is EnumConfig e)
                            {
        ...
        public static OptionBehaviour CreateOption(NumberOption prefab, ICustomRole role, NumConfig config, Transform transform)
        {
            ...
            option.MinusBtn.SetNewAction(delegate
            {
                if (num - config.ReduceValue >= 0)
                {
                    ...
                    role.RpcSetNewRoleValue(config, num, "<color=#" + ColorUtility.ToHtmlStringRGB(role.RoleColor) + ">" + role.RoleName + " (" + config.ConfigName + ")</color>: " + num.ToString() + ".");
                }
            });
        ...
        public static OptionBehaviour CreateOption(NumberOption prefab, ICustomRole role, EnumConfig config, Transform transform)
        {
            ...
            {
                config.BackValue();
                role.RpcSetNewRoleValue(config, config.ConfigEntry.Value, "<color=#" + ColorUtility.ToHtmlStringRGB(role.RoleColor) + ">" + role.RoleName + " (" + config.ConfigName + ")</color>: " + config.ConfigEntry.Value + ".");
            });
            option.PlusBtn.SetNewAction(delegate
            {
                config.NextValue();
